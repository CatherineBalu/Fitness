...
export const tbEmployee = pgTable('TB_employee', {
  ...
  employeeTypeId: uuid('ID_employee_type_fk').notNull().references(() => tbEmployeeType.id),
...
export const tbLecture = pgTable('TB_lecture', {
  ...
  exerciseTypeId: uuid('ID_exercise_type_fk').notNull().references(() => tbExerciseType.id),
...
export const tbSchedule = pgTable('TB_schedule', {
  ...
  lectureId: uuid('ID_lecture_fk').notNull().references(() => tbLecture.id),
  roomId: uuid('ID_room_fk').notNull().references(() => tbRoom.id),
...
export const tbCustomerReservation = pgTable('TB_customer_reservation', {
  ...
  customerId: uuid('ID_customer_fk').notNull().references(() => tbCustomer.id),
  scheduleId: uuid('ID_schedule_fk').notNull().references(() => tbSchedule.id),
...
export const tbPaymentHistory = pgTable('TB_payment_history', {
  ...
  customerId: uuid('ID_customer_fk').notNull().references(() => tbCustomer.id),
  subscriptionId: uuid('ID_subscription_fk').notNull().references(() => tbSubscription.id),
