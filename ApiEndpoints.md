...
- POST `/schedule` — create a new scheduled lecture (body: `{ lectureId, roomId, startTime, endTime, instructors: [{ employeeId, isLead }] }`). Rejects with 422 if `endTime <= startTime`, 409 if the room or an instructor is already booked for an overlapping time.
- PATCH `/schedule/:id` — update room, date or time for a specific instance (body: `{ roomId?, startTime?, endTime? }` where `startTime`/`endTime` are full ISO datetimes — changing the date reschedules the lecture). Same 422/409 validation as create.
